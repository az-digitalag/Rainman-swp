...
Ta <- read.csv("../data/Halfhourly_VWC_Weather_030621.csv") %>%
  ...
  mutate(TIMESTAMP = as.POSIXct(TIMESTAMP),
         ...
         D_inside = VPD(RH_inside/100, T_inside),
         D_outside = VPD(RH_outside/100, T_outside)) %>%
  rename(dt = TIMESTAMP) %>%
  tidyr::pivot_longer(!c(dt, date), 
...
For the two plots with both kinds of sensors, we combine the daily data and plot as a time-series to obtain the time periods of correlation.
...
cols <- brewer.pal(11, "Spectral")
# display.brewer.pal(11, "Spectral")
ggplot(SW_long, aes(x = date, color = Depth)) +
...
We plot the data in a 1:1 fashion. When only considering data after 7/13/2020 (when irrigation experiment began), the two deeper depths are not useable due to very low variation. 
...
ggplot(filter(SW_wide, date < as.Date("2020-07-13")), aes(y = WC_mean, x = WP_mean)) +
  ...
  facet_grid(depth~plotID,  scales = "free") +
  theme_bw(base_size = 12) +
